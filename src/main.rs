...
fn main() {
    ...
    loop {
        ...
        if let Some(data) = datos_historia.get(tag_actual){
            ...
            let mut selecction = String::new();
            std::io::stdin().read_line(&mut selecction).unwrap();
            let selecction = selecction.trim().parse().unwrap_or(99);

            if let Some(opcion_elegida) = &data.opciones.get(selecction){
